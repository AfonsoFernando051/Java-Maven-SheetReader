...
public class SaveBytesManager {
  ...
  /**
   * Data de criacao da planilha
   */
  private LocalDate dataCriacao = null;

  /**
   * @return {@link #dataCriacao}
   */
  ...
  public void setDataCriacao() {
    try (InputStream inputStream = getNewInputWithBytes()) {
      ...
      // Obtém os metadados do documento apenas se for XLSX
      if (workbook instanceof POIXMLDocument) {
        POIXMLDocument xmlDocument = (POIXMLDocument) workbook;
        Date creationDate = xmlDocument.getProperties().getCoreProperties()
            .getCreated();

        if (creationDate != null) {
          dataCriacao = creationDate.toInstant()
